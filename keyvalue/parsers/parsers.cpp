...
namespace keyvalue::parsers {
    ...
    static Statement build_from_args(DynamicArray<AutoString8>&& args) {
        ...
        if (arg_eq(args[0], "DBSIZE"))   return Statement{DbSize{}};
        if (arg_eq(args[0], "COMMAND"))  return Statement{Cmd{}};
        if (arg_eq(args[0], "FLUSHDB"))  return Statement{FlushDb{}};
        if (arg_eq(args[0], "FLUSHALL")) return Statement{FlushAll{}};
        ...
        if (arg_eq(args[0], "SET")) {
            if (args.length < 3) return Statement{Unknown{move(args[0]), {}}};
        ...
        if (arg_eq(args[0], "DEL") || arg_eq(args[0], "EXISTS") || arg_eq(args[0], "MGET")) {
            ...
            if (arg_eq(args[0], "DEL"))    return Statement{Del{move(keys)}};
            if (arg_eq(args[0], "EXISTS")) return Statement{Exists{move(keys)}};
        ...
        if (arg_eq(args[0], "SCAN")) {
            if (args.length < 2) return Statement{Unknown{move(args[0]), {}}};
            ...
            return Statement{Scan{cursor, move(match), move(count)}};
        }
        ...
        return Statement{Unknown{move(name), move(rest)}};
    }
    ...
    static Pair<ParseResult, Statement> parse_resp_array(const U8* data, U64 length, U64* consumed) {
        ...
        if (!eol) return {ParseResult::Incomplete, {}};

        S64 count = 0;
        if (!parse_decimal(p, eol, &count) || count < 0) return {ParseResult::Error, {}};
        ...
        for (S64 i = 0; i < count; i++) {
            ...
            if (!eol) return {ParseResult::Incomplete, {}};
            S64 bulk_len = 0;
            if (!parse_decimal(p, eol, &bulk_len) || bulk_len < 0) return {ParseResult::Error, {}};
            p = eol + 2;
            if (p + U64(bulk_len) + 2 > end) return {ParseResult::Incomplete, {}};
            push_back(args, AutoString8{String8{p, U64(bulk_len)}});
            p += U64(bulk_len) + 2;
        }
    ...
    static Pair<ParseResult, Statement> parse_inline(const U8* data, U64 length, U64* consumed) {
        ...
        *consumed = U64(eol - data) + 2;
        ...
        while (p < eol) {
            ...
            const U8* tok = p;
            while (p < eol && *p != ' ' && *p != '\t') ++p;
            push_back(args, AutoString8{String8{tok, U64(p - tok)}});
        }

        if (args.length == 0) return {ParseResult::Error, {}};
        return {ParseResult::Ok, build_from_args(move(args))};
    }
