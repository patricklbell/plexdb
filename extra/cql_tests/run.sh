...
fetch_tests() {
    ...
    # Install our compatibility shims into the right places
...
pip install -q cassandra-driver pytest 2>/dev/null
...
if [ ! -f "$BINARY" ]; then
    ...
    echo "Build with:  cmake -B build -G Ninja && ninja -C build objstore_server" >&2
    exit 1
fi

DB="$(mktemp -u /tmp/cql_conformance_XXXXXX.db)"
SERVER_PID=""
