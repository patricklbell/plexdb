...
TEST_CASE("append_fmt writes directly to BufferedString8", "[plexdb.string][append_fmt]") {
    SECTION("basic integer format") {
        ...
        auto flush_fn = [&flush_count, &flushed_data, &buf](const char* data, U64 length, bool) {
            flush_count++;
            flushed_data = String8{data, length};
        };
    ...
    SECTION("hex format") {
        ...
        auto flush_fn = [&flush_count](const char* data, U64 length, bool) {
            flush_count++;
        };
    ...
    SECTION("format with existing content") {
        ...
        auto flush_fn = [](const char* data, U64 length, bool) {};
    ...
    SECTION("triggers flush when buffer fills") {
        ...
        auto flush_fn = [&flush_count, &total_flushed](const char* data, U64 length, bool) {
            flush_count++;
