...
namespace plexdb::os {
    ...
    SocketResult socket_send(Handle socket, const void* data, U32 length) {
        ...
        if (err == ECONNRESET) {
            return {0, SocketError::ConnectionReset};
        }
        if (err == EPIPE) {
...
#elif PLEXDB_OS_WINDOWS
    static SOCKET handle_to_sock(Handle h) {
        return static_cast<SOCKET>(h.u64[0]);
    }
    static Handle sock_to_handle(SOCKET s) {
        return Handle{.u64 = {static_cast<U64>(s)}};
    }
    ...
    bool socket_connect(Handle socket, const char* host, U16 port) {
        ...
        addr.sin_family = AF_INET;
        addr.sin_port   = htons(port);
        if (InetPtonA(AF_INET, host, &addr.sin_addr) <= 0) {
            return false;
        }
