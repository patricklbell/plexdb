...
module plexdb.plugin;

namespace plexdb::plugin {
#ifdef PLEXDB_ENABLE_PLUGINS
...
#else // !PLEXDB_ENABLE_PLUGINS
    void dispatch(const PlexdbLogEvent&) {
    }
    uint32_t internal_register_producer(const char*) {
        return 0;
    }
    void internal_unregister_producer(uint32_t) {
    }
    uint32_t internal_register_stat(uint32_t, uint32_t, const char*) {
        return 0;
    }
...
extern "C" {
#ifdef PLEXDB_ENABLE_PLUGINS
void plexdb_plugin_register_consumer(PlexdbLogConsumerFn fn, void* ctx) {
    plexdb::plugin::register_consumer(fn, ctx);
}
void plexdb_plugin_unregister_consumer(PlexdbLogConsumerFn fn, void* ctx) {
    plexdb::plugin::unregister_consumer(fn, ctx);
}
#else
void plexdb_plugin_register_consumer(PlexdbLogConsumerFn, void*) {
}
void plexdb_plugin_unregister_consumer(PlexdbLogConsumerFn, void*) {
}
#endif
}
