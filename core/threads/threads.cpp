...
namespace plexdb::threads {
    ...
    Thread launch(AutoFunctor<void()> fn, const char* name) {
        ...
        if (name) {
            os::thread_set_name(t.handle, name);
        }
        return t;
    }
    ...
    // semaphore
    // ========================================================================
    Semaphore::Semaphore(U32 initial_count)
        : handle(os::semaphore_open(initial_count)) {
    }
    ...
    void Semaphore::signal() {
        os::semaphore_signal(handle);
    }
    void Semaphore::wait() {
        os::semaphore_wait(handle);
    }
    ...
    Scope::Scope(Arena* in_arena)
        : arena(in_arena)
        , offset(arena::offset(*arena)) {
    }
    Scope::~Scope() {
        if (this->active) {
