...
namespace plexdb::blob {
    ...
    coroutine::Task<TArrayView<B>> r_or_rw_block(BlobStaticPaged& blob, U64 offset) {
        ...
        if constexpr (IsConst<B>) {
            data = co_await pager::rpage(*blob.pager, blob.pages[page_idx]);
        } else {
            data = co_await pager::rwpage(*blob.pager, blob.pages[page_idx]);
        }

        co_return TArrayView<B>(data + offset_in_page, page_size - offset_in_page);
    }
    coroutine::Task<TArrayView<const U8>> rblock(BlobStaticPaged& blob, U64 offset) {
        co_return co_await r_or_rw_block<const U8>(blob, offset);
    }
    coroutine::Task<TArrayView<U8>> rwblock(BlobStaticPaged& blob, U64 offset) {
        co_return co_await r_or_rw_block<U8>(blob, offset);
    }

    coroutine::Task<void> remove_impl(BlobStaticPaged& blob) {
        for (U64 page : blob.pages) {
            co_await pager::delete_page(*blob.pager, page);
        }
    }
    ...
    template<typename B>
        requires Either<B, const U8, U8>
    coroutine::Task<TArrayView<B>> r_or_rw_block(BlobDynamicPaged& blob, U64 offset) {
        const auto& page_size = blob.pager->header.page_size;
        ...
        B* data;
        if constexpr (IsConst<B>) {
            data = co_await pager::rpage(*blob.pager, blob.data_pages[page_idx]);
        } else {
            data = co_await pager::rwpage(*blob.pager, blob.data_pages[page_idx]);
        }

        co_return TArrayView<B>(data + offset_in_page, length);
    }
    ...
    coroutine::Task<void> resize_impl(BlobDynamicPaged& blob, U64 new_size) {
        ...
        U64* first_data_entries = reinterpret_cast<U64*>(co_await pager::rwpage(*blob.pager, first_data_page));
        ...
        if (new_total_page_count > old_total_page_count) {
            if (old_data_page_count == 1) {
                U64 first_header_page = co_await pager::new_page(*blob.pager);
            ...
            if (new_header_pages.length <= new_header_page_count) {
                for (;;) {
                    ...
                    U64* entries = reinterpret_cast<U64*>(co_await pager::rwpage(*blob.pager, last_header_page));
                    ...
                    for (U64 entry_idx = first_entry_idx; entry_idx < last_entry_count; entry_idx++) {
                        U64 data_page = co_await pager::new_page(*blob.pager);
                    ...
                    if (new_header_pages.length < new_header_page_count) {
                        U64 header_page = co_await pager::new_page(*blob.pager);
            ...
            for (U64 header_idx = new_last_header_idx; header_idx < old_header_page_count; header_idx++) {
                ...
                U64* entries = reinterpret_cast<U64*>(co_await pager::rwpage(*blob.pager, header_page));
                ...
                for (U64 entry_idx = first_entry_idx; entry_idx < last_entry_count; entry_idx++) {
                    ...
                    co_await pager::delete_page(*blob.pager, data_page);
                }

                if (header_idx + 1 > new_header_page_count) {
                    co_await pager::delete_page(*blob.pager, header_page);
                }
            }
        }
