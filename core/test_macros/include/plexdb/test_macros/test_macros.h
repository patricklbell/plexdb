...
inline void drive_test_coroutine(plexdb::coroutine::Task<void> task) {
    task.resume();
    assert(task.done() && "io test coroutine did not complete in one resume");
}
...
#define PAGER_TEST_CASE(name, tags)                                              \
    ...
    TEST_CASE(name, tags) {                                                      \
        drive_test_pager(PLEXDB_PAGER_TEST_CAT(pager_test_, __LINE__)());        \
    }                                                                            \
    static coroutine::Task<void> PLEXDB_PAGER_TEST_CAT(pager_test_, __LINE__)()

#define IO_TEST_CASE(name, tags)                                              \
    static coroutine::Task<void> PLEXDB_PAGER_TEST_CAT(io_test_, __LINE__)(); \
    TEST_CASE(name, tags) {                                                   \
        drive_test_coroutine(PLEXDB_PAGER_TEST_CAT(io_test_, __LINE__)());    \
    }                                                                         \
    static coroutine::Task<void> PLEXDB_PAGER_TEST_CAT(io_test_, __LINE__)()
