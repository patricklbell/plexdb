...
export namespace plexdb {
    ...
    struct Array {
        ...
        struct Iterator {
            ...
            constexpr explicit Iterator(T* curr)
                : curr(curr) {
            }
        ...
        struct ConstIterator {
            ...
            constexpr explicit ConstIterator(const T* curr)
                : curr(curr) {
            }
        ...
        constexpr operator TArrayView<const T, L>() const noexcept {
            return {values, static_cast<L>(N)};
        }
        template<typename L = U64>
        constexpr operator TArrayView<T, L>() noexcept {
            return {values, static_cast<L>(N)};
        }
    ...
    struct CappedArray : Array<T, N> {
        ...
        // Iteration
        // ====================================================================
        constexpr Base::Iterator end() noexcept {
            return typename Base::Iterator{this->values + cap};
        }
        constexpr Base::ConstIterator end() const noexcept {
            return typename Base::ConstIterator{this->values + cap};
        }
        constexpr Base::ConstIterator cend() const noexcept {
            return typename Base::ConstIterator{this->values + cap};
        }
        ...
        template<typename L = U64>
        constexpr operator TArrayView<const T, L>() const noexcept {
            return {this->values, static_cast<L>(this->cap)};
        }
        template<typename L = U64>
        constexpr operator TArrayView<T, L>() noexcept {
            return {this->values, static_cast<L>(this->cap)};
    ...
    template<typename T, U64 N>
    constexpr bool empty(const RingFifo<T, N>& fifo) {
    ...
    template<typename T, U64 N>
    constexpr U64 capacity([[maybe_unused]] const RingFifo<T, N>& fifo) {
        return N;
    }
    ...
    struct MapFixedSlots {
        ...
        MapFixedSlots(Stack<Pair<K, V>>* slots, U64 slot_count)
            : slots(slots)
            , slot_count(slot_count) {
        }

        template<U64 N>
        MapFixedSlots(Stack<Pair<K, V>> slots[N])
            : slots(slots)
            , slot_count(N) {
        }
        ...
        struct Iterator {
            ...
            Iterator(MapFixedSlots<K, V>* map, U64 slot_idx, Node* node)
                : map(map)
                , slot_idx(slot_idx)
                , node(node) {
            }
        ...
        Iterator begin() noexcept {
            return Iterator{this, 0, front(slots)};
        }
        Iterator end() noexcept {
            return Iterator{this, slots->slot_count - 1, nullptr};
        }
    ...
    struct MapFixedSentinel {
        ...
        struct Iterator {
            ...
            Iterator(MapFixedSentinel<K, V, C, S>* map, U64 idx)
                : map(map)
                , slot_idx(idx) {
            ...
            Pair<K, V>& operator*() const {
                return this->map->key_values[this->slot_idx];
            }
            Pair<K, V>* operator->() {
                return &this->map->key_values[this->slot_idx];
            }
            ...
            bool operator==(const Iterator& other) const {
                return this->map == other.map && this->slot_idx == other.slot_idx;
            }
            bool operator!=(const Iterator& other) const {
                return !(*this == other);
            }
