...
Runs Apache Cassandra CQL tests (ported to Python by ScyllaDB) against `cql`.

## Quick start

```bash
# Build the server
cmake -B build -G Ninja -DPLEXDB_ENABLE_TESTS=ON -DPLEXDB_ENABLE_PLUGINS=ON -DPLEXDB_DEBUG=ON
...
|------|-------------|---------|
| `-b <path>` | plexdb_cql_server binary | `./build/cql/plexdb_cql_server` |
...
./build/cql/cql_tests --skip-benchmarks "[cql.parser],[cql.tcp]"
```
