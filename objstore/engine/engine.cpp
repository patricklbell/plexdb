...
namespace objstore::engine {
    coroutine::Task<Engine> Engine::create(Pager* in_pager) {
        Engine engine{};
        engine.pager = in_pager;
        engine.schema = co_await schema::Schema::load(in_pager, in_pager->header.root_page);
        co_return move(engine);
    }
    ...
    static auto make_buffer_writer(DynamicArray<U8>& buf) {
        ...
        co_return co_await visit(statement.value, [&engine](const auto& stmt) -> coroutine::Task<ExecutionResult> {
            ...
            if constexpr (SameAs<T, CreateKeyspace>) {
                if (is_system_keyspace(stmt.name)) {
                    co_return (stmt.if_not_exists) ? make_void_success() : make_keyspace_already_exists(stmt.name);
                }
                ...
                if (ks == nullptr) {
                    co_return make_server_error("Failed to create keyspace");
                }

                co_return make_keyspace_created(stmt.name);
                ...
                if (ks == nullptr) co_return make_keyspace_not_found(ks_name);

                if (auto existing = schema::read_table(engine.schema, *ks, stmt.name.table_name); existing.value != nullptr) {
                    co_return (stmt.if_not_exists) ? make_void_success() : make_table_already_exists(ks_name, stmt.name.table_name);
                }

                auto tbl = (co_await schema::create_table(engine.schema, *ks, stmt)).value;
                if (tbl == nullptr) {
                    co_return make_server_error("Failed to create table");
                }

                co_return make_table_created(ks_name, stmt.name.table_name);
            } else if constexpr (SameAs<T, UseKeyspace>) {
                if (is_system_keyspace(stmt.keyspace)) {
                    engine.current_keyspace = stmt.keyspace;
                    co_return make_use_keyspace(engine.current_keyspace);
                }

                auto ks = schema::read_keyspace(engine.schema, stmt.keyspace).value;
                if (ks == nullptr) co_return make_keyspace_not_found(stmt.keyspace);
                engine.current_keyspace = stmt.keyspace;
                ...
                auto ks = schema::read_keyspace(engine.schema, stmt.keyspace).value;
                if (ks == nullptr) {
                    co_return (stmt.if_exists) ? make_void_success() : make_keyspace_not_found(stmt.keyspace);
                }

                assert_true_not_implemented(stmt.options.identifier_values.length == 0, "ALTER KEYSPACE WITH options are not implemented");

                co_return make_schema_changed(stmt.keyspace);
                ...
                auto ks = schema::read_keyspace(engine.schema, ks_name).value;
                if (ks == nullptr) {
                    co_return (stmt.if_exists) ? make_void_success() : make_keyspace_not_found(ks_name);
                }

                if ((co_await schema::delete_table(engine.schema, *ks, stmt.table.table_name)).error != schema::Error::None) {
                    if (stmt.if_exists) co_return make_void_success();
                    co_return make_table_not_found(ks_name, stmt.table.table_name);
                }

                co_return make_schema_changed(ks_name, stmt.table.table_name);
                ...
                if (tbl == nullptr) co_return make_table_not_found(ks_name, stmt.table.table_name);

                co_await btree::truncate(tbl->btree);

                co_return make_void_success();
                ...
                if (tbl == nullptr) co_return make_table_not_found(ks_name, stmt.from.table_name);
                ...
                if (static_cast<bool>(pk_begin) && !io::can_cast_write_evaluated_as_column_value(*pk_begin, pk_col.type)) {
                    co_return make_where_invalid_type(ks->name, tbl->name);
                }
                ...
                co_return co_await visit(stmt.insert_clause, [&engine, ks, tbl, &ks_name, &stmt](const auto& v) -> coroutine::Task<ExecutionResult> {
                    using T = Decay<decltype(v)>;

                    if constexpr (SameAs<T, Insert::NamesValues>) {
                        if (v.names.length != v.values.length) co_return make_insert_column_does_not_match_value_count(ks->name, tbl->name);
                        ...
                        for (const auto& col : tbl->cols) {
                            ...
                            if (names_idx_opt) {
                                if (col.tombstone) co_return make_insert_into_deleted_column(ks->name, tbl->name);
                                const auto& eval = evaluate(v.values[*names_idx_opt]);
                                if (!io::can_cast_write_evaluated_as_column_value(eval, col.type))
                                    co_return make_insert_incompatible_literal(ks->name, tbl->name);
                            }
                        }

                        const auto& pk_col = tbl->cols[tbl->primary_col_idx];
                        auto pk_idx_opt = try_get_names_idx(pk_col.name);
                        if (!pk_idx_opt) co_return make_insert_missing_pks(ks->name, tbl->name);
                        ...
                        auto write = make_buffer_writer(row_buffer);
                        ...
                        blob::BlobDynamicPaged row_blob = co_await blob::BlobDynamicPaged::load(engine.pager, row_page);
                ...
                if (existing_page_opt) {
                    ...
                    {
                        ColumnIterator col_it = co_await ColumnIterator::load(engine.pager, tbl, *existing_page_opt);
                        ...
                        if (type_matches_tag<Constant>(eval.value)) {
                            ...
                            if (type_matches_tag<Null>(c.value)) {
                                ...
                                if (io::can_cast_write_evaluated_as_column_value(eval, tbl->cols[idx].type)) {
                                    ...
                                    auto w = make_buffer_writer(tmp_buffer);
                    ...
                    blob::BlobDynamicPaged row_blob = co_await blob::BlobDynamicPaged::load(engine.pager, new_row_page);
                    co_await blob::insert(row_blob, row_buffer.ptr, row_buffer.length);
                ...
                if (auto opt_row_page = co_await btree::tfind<U64>(tbl->btree, pk_key); static_cast<bool>(opt_row_page)) {
                    blob::BlobDynamicPaged row_blob = co_await blob::BlobDynamicPaged::load(engine.pager, *opt_row_page);
                ...
                auto tbl = schema::read_table(engine.schema, *ks, stmt.table.table_name).value;
                if (tbl == nullptr) {
                    co_return (stmt.if_exists) ? make_void_success() : make_table_not_found(ks_name, stmt.table.table_name);
                }

                co_return co_await visit(stmt.alter_table_instruction, [&engine, ks, tbl, &ks_name, &stmt](const auto& instr) -> coroutine::Task<ExecutionResult> {
                    ...
                    if constexpr (SameAs<I, AlterTable::AddColumnInstruction>) {
                        for (const auto& col_def : instr.column_definitions) {
                            ...
                            if (auto res = co_await schema::create_column(engine.schema, *tbl, col_def); res.error != schema::Error::None) {
                                co_return make_server_error("Failed to add column");
                            }
                        }
                        co_return make_schema_changed(ks_name, stmt.table.table_name);
