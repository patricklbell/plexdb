...
namespace {
    ...
    bool buf_contains(const U8* hay, U64 hay_len, const U8* needle, U64 needle_len) {
        ...
        for (U64 i = 0; i + needle_len <= hay_len; i++) {
            if (memory_compare(hay + i, needle, needle_len) == 0) return true;
        }
        return false;
    }
    ...
    bool recv_all(Socket& socket, U8* out, U64 byte_count) {
        ...
        while (got < byte_count) {
            ...
            if (res.byte_count == 0) return false;
            got += res.byte_count;
        }
        return true;
    }
...
// encoders
// ============================================================================
void append_u8    (DynamicArray<U8>& buf, U8 v)  { push_back(buf, v); }
...
void append_cql_bytes(DynamicArray<U8>& buf, const U8* data, S32 n) {
    append_be_s32(buf, n);
    if (n > 0) append_bytes(buf, data, U64(n));
}
...
void prepend_v4_header(DynamicArray<U8>& buf, U8 opcode, S16 stream) {
    ...
    U8 hdr[9] = {
        ...
        opcode,
        U8(U32(body_len) >> 24), U8(U32(body_len) >> 16),
        U8(U32(body_len) >> 8),  U8(U32(body_len)),
    };
...
Frame recv_frame(Socket& socket) {
    Frame f;
    U8 hdr[9] = {};
    if (!recv_all(socket, hdr, 9)) return f;
    ...
    if (f.body_len > 0) {
        ...
        if (!recv_all(socket, f.body.ptr, U64(f.body_len))) f.body.length = 0;
    }
    return f;
}
...
void append_cql_bind_s64(DynamicArray<U8>& buf, S64 v) {
    U8 b[8]; U64 u = U64(v);
    for (int i = 7; i >= 0; i--) b[i] = U8(u >> ((7-i)*8));
    append_cql_bytes(buf, b, 8);
}
void append_cql_bind_f64(DynamicArray<U8>& buf, F64 v) {
    U64 bits; memory_copy(&bits, &v, 8);
    U8 b[8];
    for (int i = 7; i >= 0; i--) b[i] = U8(bits >> ((7-i)*8));
    append_cql_bytes(buf, b, 8);
}
