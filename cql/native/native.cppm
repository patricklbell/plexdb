...
namespace cql::native {
    ...
    constexpr String8 op_codes_to_str(const U8& op) {
        switch (op) {
            ...
            case op_codes::OPTIONS:
                return "OPTIONS";
            case op_codes::SUPPORTED:
                return "SUPPORTED";
            case op_codes::QUERY:
                return "QUERY";
            case op_codes::RESULT:
                return "RESULT";
    ...
    template<typename T>
    S32 basic_element_byte_size(const type::Basic& dtype, const T& value) {
        using TT = Decay<T>;
        ...
        if constexpr (SameAs<TT, S64>) {
            ...
            assert_true(dtype == type::Basic::inet, "Inet value does not match basic type");
            return value.is_v6 ? 16 : 4;
        } else if constexpr (SameAs<TT, VarInt>) {
            assert_true(dtype == type::Basic::varint, "VarInt value does not match basic type");
            assert_not_implemented("CQL native protocol encoding for varint is not implemented");
            return 0;
        } else if constexpr (SameAs<TT, Decimal>) {
            assert_true(dtype == type::Basic::decimal, "Decimal value does not match basic type");
    ...
    template<typename T>
    concept IsCqlDA = IsDynamicArray<T>;
    template<typename T>
    concept IsCqlDS = IsDynamicSet<T>;
    template<typename T>
    concept IsCqlDM = IsDynamicMap<T>;
    ...
    template<typename T>
    void append_cql_basic_element(Frame& f, const type::Basic& dtype, const T& v) {
        using TT = Decay<T>;
        ...
        if constexpr (SameAs<TT, AutoString8>) {
            ...
            assert_true(dtype == type::Basic::uuid || dtype == type::Basic::timeuuid, "UUID value does not match type::Basic");
            ...
            assert_true(dtype == type::Basic::smallint, "S16 value does not match type::Basic");
            ...
            assert_true(dtype == type::Basic::float_, "F32 value does not match type::Basic");
            U32 bits;
            ...
            assert_true(dtype == type::Basic::double_, "F64 value does not match type::Basic");
            U64 bits;
            os::memory_copy(&bits, &v, sizeof(bits));
            U8 data[8];
            for (int i = 7; i >= 0; i--) {
            ...
            assert_true(dtype == type::Basic::inet, "Inet value does not match type::Basic");
            if (v.is_v6) {
        ...
        if constexpr (Version >= 5) {
            ...
            const auto fill_to = [&](U64 n) -> coroutine::Task<bool> {
                while (read_buf.length < n) {
                    if (!co_await try_append_tcp_read(req, read_buf)) {
                        co_return false;
                    }
                }
                co_return true;
            ...
            const auto consume_front = [&](U64 n) {
                ...
                if (rem > 0) {
                    os::memory_move(read_buf.ptr, read_buf.ptr + n, rem);
                }
                resize(read_buf, rem);
            };
            ...
            while (true) {
...

                {
                    ZoneScopedN("post_startup_loop::read")
                        ...
                        if (!co_await fill_to(V5_HDR_WITH_CRC24_BYTE_COUNT)) {
                            ok = false;
                            break;
                        }
                        ...
                        U32 hdr_crc_stored   = U32(read_buf[V5_HDR_BYTE_COUNT]) | (U32(read_buf[V5_HDR_BYTE_COUNT + 1]) << 8) | (U32(read_buf[V5_HDR_BYTE_COUNT + 2]) << 16);
                        ...
                        if (!co_await fill_to(payload_len + V5_PAYLOAD_CRC32_BYTE_COUNT)) {
                            ok = false;
                            break;
                        }
                        ...
                        // Verify CRC32 of payload
                        {
                            ...
                            U32 payload_crc_stored   = U32(read_buf[payload_len]) | (U32(read_buf[payload_len + 1]) << 8) | (U32(read_buf[payload_len + 2]) << 16) | (U32(read_buf[payload_len + 3]) << 24);
                ...
                while (p + V4_FRAME_HEADER_BYTE_COUNT <= end) {
                    ...
                    if (p + V4_FRAME_HEADER_BYTE_COUNT + U64(inner_body_len) > end) {
                        break;
                    }
            ...
            while (true) {
                ...
                {
                    ZoneScopedN("post_startup_loop::read") while (frame.length < V4_FRAME_HEADER_BYTE_COUNT) {
                        if (!co_await try_append_tcp_read(req, frame)) {
                            break;
                        }
                    }
                    if (frame.length < V4_FRAME_HEADER_BYTE_COUNT) {
                        break;
                    }
