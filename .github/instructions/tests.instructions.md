...
- Build with `-DPLEXDB_ENABLE_LOGGING=ON` so log messages appear in failing tests.
...
- Custom log consumers can be registered via the plugin ABI (`plexdb_log_register_consumer` in `core/log/log_abi.h`). Write a consumer to filter by producer ID, log level, or stat ID.
